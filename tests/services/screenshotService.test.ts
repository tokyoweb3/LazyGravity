...
describe('ScreenshotService - screenshot feature (Step 8)', () => {
    ...
    it('captures a screenshot and returns a Buffer', async () => {
        ...
        mockCdpService.callWithRetry.mockResolvedValue({
            data: dummyBase64
        });
    ...
    // ──────────────────────────────────────────────────────
    it('calls captureScreenshot with callWithRetry and correct parameters', async () => {
        mockCdpService.callWithRetry.mockResolvedValue({ data: dummyBase64 });

        screenshotService = new ScreenshotService({ cdpService: mockCdpService });
        await screenshotService.capture();

        expect(mockCdpService.callWithRetry).toHaveBeenCalledWith(
    ...
    it('returns success:false with error message on CDP error', async () => {
        mockCdpService.callWithRetry.mockRejectedValue(new Error('CDP connection error'));
        ...
        expect(result.error).toContain('CDP connection error');
    });
    ...
    it('returns success:false when CDP returns empty data', async () => {
        mockCdpService.callWithRetry.mockResolvedValue({ data: '' });
